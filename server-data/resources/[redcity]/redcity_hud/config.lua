Config = {}
Config.UseRealTime = true

Config.HUD = {
    VehiclePosition = 'bottom-center',
    Scale = 0.85,
    Theme = 'redcity_gold',
    HideRadarOnFoot = true,
    ShowPlayerInfoOnFoot = true,
    ShowStatusHUD = true,
    VehicleUpdateInterval = 180,
    StatusUpdateInterval = 750,
    OnFootUpdateInterval = 1000,
    MaxSpeed = 240
}

Config.Seatbelt = {
    Enabled = true,
    Key = 'B',
    WarningSpeed = 30.0,
    PlaySound = true,
    SoundCooldown = 1800
}

Config.Voice = {
    Enabled = true,
    Key = 'Z',
    Modes = {
        { name = 'Small', label = 'Whisper', color = '#38BDF8' },
        { name = 'Normal', label = 'Normal', color = '#FFD21F' },
        { name = 'Shout', label = 'Shout', color = '#FB923C' }
    }
}

Config.VehicleDamage = {
    Enabled = true,
    EngineFailHealth = 100.0,
    WarningHealth = 300.0,
    DisableDrivingOnFailure = true
}

Config.Fuel = {
    Enabled = true,
    PricePerLiter = 15,
    MaxFuel = 100.0,
    BaseConsumption = 0.015,
    SpeedMultiplier = 0.0008,
    RpmMultiplier = 0.02,
    IdleConsumption = 0.003,
    UpdateInterval = 3000,
    EmptyEngineCutoff = 0.8,
    PaymentAccount = 'money',
    RefuelTick = 900,
    RefuelLitersPerTick = 1.5
}

Config.GasStations = {
    vec3(49.4187, 2778.793, 58.043),
    vec3(263.894, 2606.463, 44.983),
    vec3(1039.958, 2671.134, 39.550),
    vec3(1207.260, 2660.175, 37.899),
    vec3(2539.685, 2594.192, 37.944),
    vec3(2679.858, 3263.946, 55.240),
    vec3(2005.055, 3773.887, 32.403),
    vec3(1687.156, 4929.392, 42.078),
    vec3(1701.314, 6416.028, 32.763),
    vec3(179.857, 6602.839, 31.868),
    vec3(-94.4619, 6419.594, 31.489),
    vec3(-2554.996, 2334.402, 33.078),
    vec3(-1800.375, 803.661, 138.651),
    vec3(-1437.622, -276.747, 46.207),
    vec3(-2096.243, -320.286, 13.168),
    vec3(-724.619, -935.163, 19.213),
    vec3(-526.019, -1211.003, 18.184),
    vec3(-70.2148, -1761.792, 29.534),
    vec3(265.648, -1261.309, 29.292),
    vec3(819.653, -1028.846, 26.403),
    vec3(1208.951, -1402.567, 35.224),
    vec3(1181.381, -330.847, 69.316),
    vec3(620.843, 269.100, 103.089),
    vec3(2581.321, 362.039, 108.468),
    vec3(176.631, -1562.025, 29.263),
    vec3(-319.292, -1471.715, 30.549),
    vec3(1784.324, 3330.550, 41.253)
}

Config.Interaction = {
    DrawDistance = 22.0,
    MarkerDistance = 14.0,
    RefuelDistance = 4.0,
    Key = 38,
    KeyLabel = 'E'
}
